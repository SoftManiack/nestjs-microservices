...
export class UserRepository {
    ...
    async findAndCount(params: SearchUserParams): Promise<{ items: UserEntity[]; total: number }>{

        console.log('find and count')
        console.log(params)

        const [ items, total ] = await this.gb(params).getManyAndCount()
    ...
    gb(params: SearchUserParams = {}, alias = 'user'): SelectQueryBuilder<UserEntity> {
        const query = this.userRepository.createQueryBuilder(alias);
        ...
        if(params?.phones?.length){
            query.andWhere(`${alias}.userId in (:...phones)`, { phones: params.phones })
        }
